...
export class EmployeeList extends LitElement {
  ...
  confirmDelete() {
    if (this.employeeToDelete) {
      ...
      this.showFieldNotification(msg('Employee deleted successfully'), 'success');
    }
  }
                ...
                <div class="confirm-dialog">
                  <p>
                    ...
                    <strong
                      ...
                      ${this.employeeToDelete?.lastName}?</strong
                    >
                  </p>
                  <button class="-proceed" @click="${this.confirmDelete}">
                    ${msg('Proceed')}
                  </button>
  ...
  renderTable() {
    return html`
                ...
                (emp) => html`
                  <tr>
                    ...
                    <td class="actions-container">
                      <button
                        class="action-button"
                        @click="${() => this.navigateToEditPage(emp)}"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          viewBox="0 0 24 24"
                        ...
                        >
                          <path
                            d="M3 21h3.75L17.81 9.94l-3.75-3.75L3 17.25V21zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34a1 1 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"
                          />
                        </svg>
                      </button>
                      <button
                        class="action-button"
                        @click="${() => this.openDeleteDialog(emp.id)}"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          viewBox="0 0 24 24"
      ...
      <div class="pagination-controls">
        ...
        <span>${msg(html`Page ${this.currentPage} of ${this.totalPages}`)}</span>
        <button
          class="next"
    ...
    return html`
      ...
      <div class="employee-list">
        ${this.filteredEmployees.length === 0
          ? html`<div class="no-records-message">${msg('No records found')}</div>`
          : this.filteredEmployees.map(
              (emp) => html`
                <div class="employee-item">
                  <ul>
                    <li><strong>${msg('First Name')}:</strong> ${emp.firstName}</li>
                    <li><strong>${msg('Last Name')}:</strong> ${emp.lastName}</li>
                    <li>
                      <strong>${msg('Date of Employment')}:</strong>
                      ${emp.dateOfEmployment}
                    </li>
                    <li><strong>${msg('Date of Birth')}:</strong> ${emp.dateOfBirth}</li>
                    <li><strong>${msg('Phone')}:</strong> ${emp.phone}</li>
                    <li><strong>${msg('Email')}:</strong> ${emp.email}</li>
                    <li><strong>${msg('Department')}:</strong> ${emp.department}</li>
                    <li><strong>${msg('Position')}:</strong> ${emp.position}</li>
                  </ul>
